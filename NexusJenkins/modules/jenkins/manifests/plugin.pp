...
$plugin_extention = "${name}.${plugin_ext}"
  if (empty(grep([ $::jenkins_plugins ], $search))) {
   ...
   file { "${::jenkins::plugin_dir}/${plugin}.disabled":
      ...
      group   => $::jenkins::group,
      mode    => '0644',
      require => Exec["${plugin_extention}-plugin-installlation"],
  ...
  exec { "${plugin_extention}-plugin-installlation":
    command     => "/bin/bash -c \"/usr/bin/java -jar /usr/share/jenkins/jenkins-cli.jar -s http://127.0.0.1:8080 install-plugin ${plugin}\"",
    logoutput   => 'true',
    cwd         => "/var/lib/jenkins",
    user        => "jenkins",
    unless      => "test -e /var/lib/jenkins/plugins/${plugin}*",
    provider    => shell,
